...
echo "[daily_verification] Build"
cd "$ROOT"
if [ -x node_modules/.bin/react-scripts ]; then
  CI=1 npm run build
else
  echo "[daily_verification] Installing dependencies"
  npm install
  CI=1 npm run build
fi

echo "[daily_verification] Git status"
git add api/health.js api/debug/dependencies.js scripts/daily_verification.sh || true
...
echo "[daily_verification] Push"
git push origin main

echo "[daily_verification] Waiting for Vercel deploy to settle"
sleep 18

echo "[daily_verification] Verify endpoints"
